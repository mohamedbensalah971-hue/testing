...
class ContactMergeUtilTest {
    ...
    @Test
    fun testMergeDuplicateContactsEmptyList() {
        val mergedContacts = ContactMergeUtil.mergeDuplicateContacts(emptyList())
        assertTrue(mergedContacts.isEmpty())
    }

    @Test
    fun testFindSimilarContactsNoMatches() {
        val targetContact = Contact(4, "Unknown", "5555555555", "unknown@example.com", 1643723400, emptyList())
        val similarContacts = ContactMergeUtil.findSimilarContacts(targetContact, contacts)
        assertTrue(similarContacts.isEmpty())
    }
}